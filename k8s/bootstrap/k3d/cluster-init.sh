...
# 2. 설치 여부 확인
echo "🔍 필수 도구 설치 여부를 확인합니다..."
for tool in docker k3d kubectl helm; do
    if ! command -v "$tool" &> /dev/null; then
        echo "❌ $tool 가 설치되어 있지 않습니다. 설치 후 다시 시도하세요."
        exit 1
    fi
done
...
if ! docker info &> /dev/null; then
    echo "❌ Docker 엔진이 실행 중이 아닙니다. Docker를 먼저 실행하세요."
    exit 1
fi

echo "🚀 k3d 클러스터($CLUSTER_NAME) 구축을 시작합니다..."

# 4. 기존 클러스터 존재 여부 확인 및 삭제
echo "🧹 기존 클러스터 및 레지스트리 정리 중..."
k3d cluster delete "$CLUSTER_NAME" &> /dev/null || true
docker rm -f "k3d-$REGISTRY_NAME" &> /dev/null || true

# 5. 클러스터 생성
echo "🛠️ 클러스터 생성을 시작합니다..."
CONFIG_FILE="$BASE_DIR/k3d-config.yaml"

if [ ! -f "$CONFIG_FILE" ]; then
    echo "❌ 설정 파일이 존재하지 않습니다: $CONFIG_FILE"; exit 1
fi
